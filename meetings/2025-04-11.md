# NetworkX Developer Discussions: Apr 11, 2025
...
**In attendance**: Dan Schult, Ross, Erik Welch, Rick Ratzel
...
Let's try only bringing up topics that someone in the meeting wants to discuss. If we stop early it gives more time for review. We'll try it and see how it works.

## Upcoming meetings and events
- we have 8 GSOC 2025 proposals (submitted Apr 8) and will review and decide what to do by a few days before Apr 29  (the numfocus org has to turn it in by Apr 29).
...
- visualization. We decided last fall to go with a new function name. Now suggestion is to try to combine the new functionality into the current implementation. Let's figure out what we want. :)
- Pypi org governance
- Astar variants -- lay-of-the-land with unified interface
- Is prepping objects for JSON the same as writing ascii files for specific formats?
- Explore spring_layout function signature and names for keyword args. What about the new proposed `method` kwarg.

## Topics (discussed only if someone points them out to the group during the meeting)
