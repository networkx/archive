# NetworkX Developer Discussions: April 18th, 2023

Previous meeting notes available [here](https://github.com/networkx/archive/tree/main/meetings). Please feel free to add topics for discussion and news items below.

**In attendance:** Ross, Paula, Kavish, Matt, Navya, Dan, Jarrod, Jim, Mridul
...
* SciPy conference earlybird registration deadline April 23  (event is July 9-16)
  * NetworkX/dispatching tutorial has been waitlisted
* PyCon Colombia has accepted a tutorial (early June) :tada: 
* GSoC Application Deadline: April 4, our ranking April 20 to NumFOCUS
...
* circleci artifact redirector followup
  * Both networkx and nx-guides. 

## Topics
 
- GSoC and Outreachy applicants reviews.

- Graph girth: https://github.com/networkx/networkx/pull/6633
  * Seems like a meaningful measure - what do the experts think?
  * Girth == the smallest-sized cycle
    - Kelly recently did a lot of work revamping simple cycles
    - Sentiment from everyone seems to be positive!
  
- `codecov` rugpull
  * Hopefully resolved by #6635
  * This one *will* affect old PRs - pushes to PRs that were opened prior to ~2weeks ago will likely fail. The fix is to merge with `main`
  
- Other CI maintenance: fix for circleCI artifact redirector
  * fix is straightforward, but need to add secrets to github
    - for org-wide secrets, only admins have the privileges.
    - could also do per-repo secrets (only `networkx` and `nx-guides`) if we want (I think)

- Attempt at boosting coverage for `semiconnected` uncovered weird behavior:
  * https://github.com/networkx/networkx/pull/6645

  ```python
  >>> G = nx.path_graph(100, create_using=nx.DiGraph)
  >>> nx.is_semiconnected(G)  # When no ordering is specifi
  True
  >>> nx.is_semiconnected(G, topo_order=list(range(100)))
  False
  ```
  
  A couple things from the conversations:
    - `nx.condensation` (and potentially others) remaps the nodes: look into this further - should they be remapped? Should behavior be changed? What other functions do this?
    - `is_semiconnected` works with directed networks that are not DAGs. having a topo_order input argument doesn't make sense. This should be looked into. 
    - **The action item we're sure about:** deprecate `topo_order` kwarg
