# NetworkX Developer Discussions: January 24th, 2024

Previous meeting notes available [here](https://github.com/networkx/archive/tree/main/meetings). Please feel free to add topics for discussion and news items below.

**In attendance**: Rick, Ross, Erik, Dan, Aditi, Matt
...
- could nx-parallel interact with other backends?
    - Some libraries e.g. LAPACK may need careful CI configuration to work with joblib
    - Using e.g. cugraph from within nx-parallel might be harder due to communication between joblib parallel and GPU parallel
- caching in dispatching
    ...
    - make limits for how many things get stored?
     ...
     ```python
      with nx.backend.use("graphblas", cache=True):
        nx.foo
     ```
...
- Update `not_implemented_for` to support multiple arguments?
  * https://github.com/networkx/networkx/pull/7227#discussion_r1464333522
  * Sounds like positive response all around

- Idea/Question: scipy implementations of algorithms via "internal" backend
  * An idea for e.g. the "pagerank" problem
    - Used to be a `pagerank` (pure Python), `pagerank_numpy` and `pagerank_scipy` public functions ([a few other examples](https://github.com/networkx/networkx/issues/4371))
  * Scipy is a "default" dependency - have a scipy backend shipped with networkx?

- Keep attempting to make `PlanarEmbedding` a full-featured class?
  * Further updates to add more support for working directly with
    `PlanarEmbedding` instances rather than as an output of `check_planarity`
  * https://github.com/networkx/networkx/pull/7224
  * Takeaway: make sure that PlanarEmbedding is indeed always planar - as long as we're careful about making sure that modifications to PlanarEmbedding instances don't invalidate the planarity.

- `rich_club_coefficient`: quick double-check on behavior when `normalized=True`
  * "normalize" means compute the `rcc` for a graph with a random degree distribution - however, it's not possible to generate a "random" degree distribution for graphs with fewer than 4 nodes.
  * Ref: https://en.wikipedia.org/wiki/Rich-club_coefficient
  * PR : https://github.com/networkx/networkx/pull/7212
  * 
