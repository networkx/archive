# NetworkX Developer Discussions: March 6th, 2024
...
- Opinions about exception types:
  * https://github.com/networkx/networkx/pull/7245
      - choice between NetworkXError and NetworkXSomethingElse and KeyError. 
      - Probably don't want to construct NXKeyError, NXValueError, NXTypeError, etc.
      - Probably don't want to wrap every dunder on the base graph classes in a try/except just to give a cleaner message. (But maybe we do?)
      - https://github.com/networkx/networkx/issues/1705 
      - Decision: keep it a KeyError but give it a better message. This getitem case is OK to wrap in try/except because it is a dict-like lookup, so the Key is the entire edge -- not one node.
- `nx.config`
...
- Removing animation from spectral clustering gallery example:
  * ~~https://github.com/networkx/networkx/pull/7328~~
  * Merging should drop our doc build times in CI by 200+ seconds
  * [Gallery page from PR](https://output.circle-artifacts.com/output/job/3613f76b-370f-4d30-95f4-3a05c9996921/artifacts/0/doc/build/html/auto_examples/algorithms/plot_image_segmentation_spectral_graph_partiion.html#sphx-glr-auto-examples-algorithms-plot-image-segmentation-spectral-graph-partiion-py)
